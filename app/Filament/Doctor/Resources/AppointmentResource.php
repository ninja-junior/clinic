...
class AppointmentResource extends Resource
{
        ...
        return $form
            ->schema([
                Forms\Components\Select::make('patient_id')
                    ...
                    ->required()
                    ->helperText(fn () 
                        => Filament::getTenant()->patients->isEmpty() ? new HtmlString(
                            '<span class="text-sm text-danger-600 dark:text-danger-400">No patients available for this clinic.</span>'
                        ) : '')
                    ->columnSpanFull()
                    ->getSearchResultsUsing(function (string $search) {
                        $patients = Patient::where('name', 'like', "%{$search}%")->limit(50)->get();
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\ImageColumn::make('patient.avatar')
                    ->label('Image')
                    ->circular(),
